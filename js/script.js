...
function addPagination(list) {
   // Math.ciel because we will need extras on final page
   ...
   // Traverse to the UL to get to the first button and add an active class
   if (list.length > 0) {
      let liList = ulPagination.firstElementChild;
      let firstButton = liList.firstElementChild;
      firstButton.className = "active";
   } else {
      ulOfStudents.innerHTML = "<h1 style='text-align: center; font-size: 50px;'>No Items to Display</h1>";
   }
...
//          ul.insertAdjecentHTML('beforeend', li);